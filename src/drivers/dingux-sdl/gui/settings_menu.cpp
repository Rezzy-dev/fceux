...
static MenuEntry
	settings_menu[] = {
		...
		{ "Video", "Change video config", cmd_video_settings },
		{ "Sound", "Change sound config", cmd_sound_settings },
		{ "Controls", "Change control config", cmd_control_settings },
...
int RunSettingsMenu() {
	...
	while (!done) {
		...
		if (g_dirty) {
			...
			DrawChar(gui_screen, SP_SELECTOR, 0, 0);
			DrawChar(gui_screen, SP_SELECTOR, 81, 0);
			...
			DrawText(gui_screen, "Settings", 8, 0);
